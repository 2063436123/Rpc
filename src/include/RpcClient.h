...
public:
    ...
    //template<typename Ret, typename ...Args> requires is_void<Ret>
    //void async_call(std::function<void(ErrorCode error_code, Ret)> cb, const std::string &service_name,
    //           Args... args) {
    //    async_call<Ret>(DEFAULT_REQUEST_TIMEOUT, cb, service_name, args...);
    //}
    //
    //template<typename Ret, typename ...Args> requires non_void<Ret>
    //void async_call(std::function<void(ErrorCode error_code)> cb, const std::string &service_name,
    //           Args... args) {
    ...
    template<typename Ret, typename ...Args>
    typename std::enable_if_t<!std::is_void_v<Ret>>
    async_call(size_t timeout, std::function<void(ErrorCode, Ret)> cb, const std::string &service_name,
               Args... args) {
        uint64_t request_id = next_request_id_++;
    ...
    template<typename Ret, typename ...Args>
    typename std::enable_if_t<std::is_void_v<Ret>>
    async_call(size_t timeout, std::function<void(ErrorCode)> cb, const std::string &service_name,
               Args... args) {
        uint64_t request_id = next_request_id_++;
        std::cout << "add request_id: " << request_id << std::endl;
...
private:
    void start_heartbeat() {
        ...
        auto &helper_handler = (reinterpret_cast<RpcMeta *>(main_conn->data()))->heartbeat_helper_handler;
    ...
    void handle_response(TcpConnection *conn) {
        for (const auto& x : sent_requests_)
            std::cout << x.first << " ";
        std::cout << std::endl;
        // 1. 验证header并提取body
